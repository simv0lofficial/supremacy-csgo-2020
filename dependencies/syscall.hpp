...
namespace SystemCall
{
   ...
   __forceinline uint8_t* locate_wow64_transition( ) {
	  ...
	  if ( !wow64_transition )
		 ...
		 get_module_handle( hash_32_fnv1a_const( "win32u.dll" ) ), hash_32_fnv1a_const(  "Wow64Transition" ) ) );
   ...
   __forceinline syscall_fn extract_syscall( const uint32_t function ) {
	  ...
	  if ( !verify_integrity( "C:\\Windows\\SysWOW64\\ntdll.dll" ) )
		 return fn;

	  std::ifstream file( "C:\\Windows\\SysWOW64\\ntdll.dll", std::ios::binary | std::ios::ate );
