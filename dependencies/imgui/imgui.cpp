...
void ui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
    ...
    if (text != text_end)
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, ImColor(170 / 255.f, 170 / 255.f, 170 / 255.f, g.Style.Alpha), text, text_end, wrap_width);
        if (g.LogEnabled)
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    // note: simv0l - commented this asserts because they dont like our custom drawlist.
...
void ui::Render(ImDrawList* our_list)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
        ...
        {           
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip)) {
                    ...
                    if (flags & ImGuiWindowFlags_Confirmation)
                    {
                        ImU32 bg_col = ImColor(12 / 255.f, 12 / 255.f, 12 / 255.f, settings::m_stored_alpha);
                        float alpha = 1.0f;
                        ...
                        outline(window, 0, ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, settings::m_stored_alpha));
                        outline(window, 1, ImColor(48 / 255.f, 48 / 255.f, 48 / 255.f, settings::m_stored_alpha));

                        window->DrawList->AddText(window->Pos + ImVec2(6 + 4 + 2, 6 + 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, settings::m_stored_alpha), "confirmation");
                    ...
                    {
                        ImU32 bg_col = ImColor(12 / 255.f, 12 / 255.f, 12 / 255.f, g.Style.Alpha);
                        float alpha = 1.0f;
                        if (g.NextWindowData.BgAlphaCond != 0)
                            alpha = g.NextWindowData.BgAlphaVal;
                        if (alpha != 1.0f)
                        ...
                        outline(window, 0, ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha));
                        outline(window, 1, ImColor(48 / 255.f, 48 / 255.f, 48 / 255.f, g.Style.Alpha));


                        std::string str(__DATE__);
                        window->DrawList->AddText(window->Pos + ImVec2(6 + 2 + 4 + 2 - 3, 6 + 2), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha), "supremacy");
                        window->DrawList->AddText(window->Pos + ImVec2(6 + 3 + 4 + 2 + 50 + 5 + 5 + 10 - 20, 6 + 2), ImColor(135 / 255.f, 135 / 255.f, 135 / 255.f, g.Style.Alpha), " | ");
                        window->DrawList->AddText(window->Pos + ImVec2(6 + 3 + 4 + 2 + 50 + 5 + 5 + 10 - 20 + 8, 6 + 2), ImColor(142 / 255.f, 142 / 255.f, 142 / 255.f, g.Style.Alpha), str.c_str());
                    ...
                    if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_ChildFrame) && !(flags & ImGuiWindowFlags_Tooltip)) {
                        ...
                        ImU32 bg_col = ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, g.Style.Alpha);
                        float alpha = 1.0f;
                        if (g.NextWindowData.BgAlphaCond != 0)
                            alpha = g.NextWindowData.BgAlphaVal;
                        if (alpha != 1.0f)
                            bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
                        window->DrawList->AddRectFilled(window->Pos - ImVec2(5, 0) + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size - ImVec2(-5, -5), bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);

                        window->DrawList->AddRect(window->Pos - ImVec2(5, 0), window->Pos + window->Size - ImVec2(-5, -5), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));

                        window->DrawList->AddRect(window->Pos - ImVec2(5 + 1, 0 + 1), window->Pos + window->Size - ImVec2(-5 - 1, -5 - 1), ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
                    }
                }
            }
...
void ui::BeginTooltip()
{
    ...
    if (g.DragDropWithinSourceOrTarget)
    {
        ...
        if (settings::m_is_opened)
            SetNextWindowPos(tooltip_pos);
        else
            SetNextWindowPos(ImVec2(-9000,-9000)); // it works bro stfu
...
void ui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
	...
	SetNextWindowSize(ImVec2(100 + 20, 25));
...
const ImGuiPayload* ui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect) && payload.Preview)
    {
        ...
        r.Max.x -= 3;
        r.Max.y -= 1;
        window->DrawList->AddRect(r.Min, r.Max, GetColorU32(ImGuiCol_DragDropTarget));
...
// By default, tree nodes are automatically opened during logging::get().
//-----------------------------------------------------------------------------
