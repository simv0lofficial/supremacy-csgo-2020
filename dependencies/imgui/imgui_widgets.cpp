...
void ui::TextUnformatted(const char* text, const char* text_end)
{
    ...
    const ImVec2 text_pos(window->DC.CursorPos.x + 16 + 6, window->DC.CursorPos.y + window->DC.CurrentLineTextBaseOffset);
...
void ui::TabButton3(const char* label, const char* label2, int* selected, int num, int total) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 pos = window->Pos + ImVec2(10 - 46 + 2 + 230 + 12 + 10 - 15 - 5 - 17 - 10 - 10 - 100 - 54, 9 + 24 - 20 + 30) + ImVec2((window->Size.x + 200 - 12 - 12 - 90 - 30 - 100 + 5 - 40 + 10 - 4 - 5 + 2 + 25 + 15 + 25 + 15 + 5) / total * num, 0) - ImVec2(0, 1);


    if (total == 1)
        pos = window->Pos + ImVec2(10 - 46 + 2 + 230 + 12 + 10 - 15 - 5 - 100 - 54 + 20, 9 + 24 - 20 + 30) + ImVec2((window->Size.x + 200 - 12 - 12 - 90 - 30 - 100 + 5 - 40 + 10 - 4 - 5 + 2 + 25 + 15) / total * num, 0) - ImVec2(0, 1);
    else
        pos = window->Pos + ImVec2(10 - 46 + 2 + 230 + 12 + 10 - 15 - 5 - 100 - 54, 9 + 24 - 20 + 30) + ImVec2((window->Size.x + 200 - 12 - 12 - 90 - 30 - 100 + 5 - 40 + 10 - 4 - 5 + 2 + 25 + 15) / total * num, 0) - ImVec2(0, 1);

    ImVec2 size = ImVec2((window->Size.x - 30 - 150 - 60 - 40 + 90 + 75) / total, 45);
    ...
    ImColor textColor = ImColor(140 / 255.f, 140 / 255.f, 140 / 255.f, g.Style.Alpha);
    if (hovered)
        textColor = ImColor(185 / 255.f, 185 / 255.f, 185 / 255.f, g.Style.Alpha);
    if (*selected == num)
        textColor = ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha);

    if (total == 1)
    {
        window->DrawList->AddText(ImVec2(pos.x - 4 + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1 - 3), textColor, label);
        ui::PushFont(ui::GetIO().Fonts->Fonts[2]);

        window->DrawList->AddText(ImVec2(pos.x - 4 + size.x / 2 - label_size.x / 2 + 9 + 3, pos.y + size.y / 2 - label_size.y / 2 - 1 + 21 - 3), textColor, label2);
    }
    else
    {
        window->DrawList->AddText(ImVec2(pos.x - 4 + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1 - 3), textColor, label);
...
void ui::TabButton2(const char* label, int* selected, int num, int total) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 pos = window->Pos + ImVec2(10 + 2 + 230 + 12 + 10 - 15 - 5 - 30 - 100 - 5 - 70 + 80, 9 + 24 - 20 - 18 - 3 + 10 - 1 + 6 - 14 + 2) + ImVec2((window->Size.x - 12 - 12 - 90 - 30 - 100 + 5 - 40 + 10 - 4 - 5 + 2 + 25 + 15 - 104) / total * num / 2, 0) - ImVec2(125, 1);
    
    ImVec2 size = ImVec2((window->Size.x - 30 - 150 - 60 - 40 + 15) / total, 28);
    ...
    window->DrawList->AddText(ImVec2(pos.x + size.x / 2 - label_size.x / 2 + 9 + 3 + 100 + 5 - 80, pos.y + size.y / 2 - label_size.y / 2 - 1 + 21 - 3 - 10), textColor, label);
...
void ui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    ImVec2 pos = window->Pos + ImVec2(10 + 2, 9 + 24) + ImVec2((window->Size.x - 12 - 12) / total * num, 0) - ImVec2(0, 1);
    ImVec2 size = ImVec2((window->Size.x - 30 + 1) / total, 28);
    ...
    //  fgui::color(41, 41, 41, 255), fgui::color(15, 15, 15, 255)
    if (*selected != num) {
       
        window->DrawList->AddRectFilledMultiColor(pos, pos + size, ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha), ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha), ImColor(15 / 255.f, 15 / 255.f, 15 / 255.f, g.Style.Alpha), ImColor(15 / 255.f, 15 / 255.f, 15 / 255.f, g.Style.Alpha));
    }
    else {
        window->DrawList->AddRectFilledMultiColor(pos, pos + size, ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha), ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha), ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, g.Style.Alpha), ImColor(24 / 255.f, 24 / 255.f, 24 / 255.f, g.Style.Alpha));
    }

    window->DrawList->AddRect(pos - ImVec2(1, 1), pos + size + ImVec2(1, 1), ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
    window->DrawList->AddRect(pos, pos + size, ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha));
...
bool ui::ButtonExLmao(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(16 + 6, 0);
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrentLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, ImClamp(window->Size.x - 64 - 6, 50.f, 200.f - 6.f), 22);
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
    ...
    window->DrawList->AddText(bb.Min + ImVec2((bb.Max.x - bb.Min.x) / 2 - label_size.x / 2, (bb.Max.y - bb.Min.y) / 2 - label_size.y / 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), label);
...
bool ui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrentLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, ImClamp(window->Size.x - 64 - 5, 50.f, 200.f - 6.f), 22);
...
bool ui::ButtonExT(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ui::ButtonT(const char* label, const ImVec2& size_arg, int page, int in, ImColor clr, bool border) {
    return ButtonExT(label, size_arg, 0, page, in, border, clr);
}



bool ui::ButtonText(const char* label, const ImVec2& size_arg)
{
    return ButtonExLmao(label, size_arg, 0);
}
...
bool ui::Checkbox(const char* label, bool* v, bool safe)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (*v)
        window->DrawList->AddRectFilledMultiColor(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha));
        //window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha));
    else {
        if (IsItemHovered())
            window->DrawList->AddRectFilledMultiColor(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(79 / 255.f, 79 / 255.f, 79 / 255.f, g.Style.Alpha), ImColor(79 / 255.f, 79 / 255.f, 79 / 255.f, g.Style.Alpha), ImColor(46 / 255.f, 46 / 255.f, 46 / 255.f, g.Style.Alpha), ImColor(46 / 255.f, 46 / 255.f, 46 / 255.f, g.Style.Alpha));
        else
            window->DrawList->AddRectFilledMultiColor(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(69 / 255.f, 69 / 255.f, 69 / 255.f, g.Style.Alpha), ImColor(69 / 255.f, 69 / 255.f, 69 / 255.f, g.Style.Alpha), ImColor(36 / 255.f, 36 / 255.f, 36 / 255.f, g.Style.Alpha), ImColor(36 / 255.f, 36 / 255.f, 36 / 255.f, g.Style.Alpha));
    ...
    window->DrawList->AddRect(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
    window->DrawList->AddText(pos + ImVec2(14 + 8, -1), safe ? ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha) : ImColor(180 / 255.f, 180 / 255.f, 60 / 255.f, g.Style.Alpha), label);
...
bool ui::BeginCombo2(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = ImClamp(window->Size.x - 48, 155.f, 200.f + 16.f - 30 - 13);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(16 + 6, 0 - 3), window->DC.CursorPos + ImVec2(w, label_size.x > 0 ? label_size.y + 25 : 25));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0, -7));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb))
        return false;
    ...
    if (!hovered)
        window->DrawList->AddRectFilledMultiColor(cb_bb.Min, cb_bb.Max - ImVec2(0, 3),
            ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, g.Style.Alpha),
            ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, g.Style.Alpha),
            ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, g.Style.Alpha),
            ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, g.Style.Alpha));
    else
        window->DrawList->AddRectFilledMultiColor(cb_bb.Min, cb_bb.Max - ImVec2(0, 3),
            ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha),
            ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha),
            ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha),
            ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha));

    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max - ImVec2(0, 3), ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
    window->DrawList->AddRect(cb_bb.Min + ImVec2(1, 1), cb_bb.Max - ImVec2(0 + 1, 3 + 1), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));

    if (label_size.x > 0)
        window->DrawList->AddText(frame_bb.Min + ImVec2(0, 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), label);

    window->DrawList->PushClipRect(cb_bb.Min, cb_bb.Max - ImVec2(20, 0), true);
    window->DrawList->AddText(cb_bb.Min + ImVec2(8, 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), preview_value);
    window->DrawList->PopClipRect();

    if (!popup_open)
    {
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 2 + 1, 12 + 2), cb_bb.Max + ImVec2(0 - 5 - 6 + 1, 3 - 8 - 6 - 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 1 - 2, 12 - 1 + 2), cb_bb.Max + ImVec2(0 - 5 - 6 + 1 - 2, 3 - 8 - 6 - 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 2 - 2, 12 - 2 + 2), cb_bb.Max + ImVec2(0 - 5 - 6 - 2 + 1 - 1, 3 - 8 - 6 - 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
    }
    else
    {
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 2 + 1, 12 + 2 - 2), cb_bb.Max + ImVec2(0 - 5 - 6 + 1, 3 - 8 - 6 - 2 + 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 1 - 2, 12 - 1 + 2), cb_bb.Max + ImVec2(0 - 5 - 6 + 1 - 2, 3 - 8 - 6 - 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(cb_bb.Max - ImVec2(16 - 2 - 2, 12 - 2 + 2), cb_bb.Max + ImVec2(0 - 5 - 6 - 2 + 1 - 1, 3 - 8 - 6 - 2), ImColor(108 / 255.f, 108 / 255.f, 108 / 255.f, g.Style.Alpha));
    }
    ...
    //if (!popup_open)
    //    RenderArrow(cb_bb.Max - ImVec2(16, 12), ImGuiDir_Down, 0.7f);
    //else
    //    RenderArrow(cb_bb.Max - ImVec2(16, 12), ImGuiDir_Up, 0.8f);
    ...
    SetNextWindowSize(ImVec2(w - 16 - 20 + 8 - 4 - 2, CalcMaxPopupHeightFromItemCount(items)) - ImVec2(0, 3));
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImRect r_outer = GetWindowAllowedExtentRect(popup_window);
            ImVec2 pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, frame_bb, ImGuiPopupPositionPolicy_ComboBox);
            SetNextWindowPos(pos + ImVec2(6, 0));
        }

    // Horizontally align ourselves with the framed text
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(style.FramePadding.x - 5, style.WindowPadding.y));
    bool ret = Begin(name, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
...
bool ui::SingleSelect2(const char* label, int* current_item, std::vector<const char*> items) {
    *current_item = ImClamp(*current_item, 0, int(items.size() - 1));

    int old_item = *current_item;

    if (ui::BeginCombo2(label, items.at(*current_item), 0, items.size())) {
...
bool ui::MultiSelect(const char* label, std::unordered_map<int, bool>* data, std::vector<const char*> items) {
    ...
    if (ui::BeginCombo(label, preview_str.size() == 0 ? "-" : preview_str.c_str(), 0, items.size())) {
        for (int i = 0; i < items.size(); i++)
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(16 + 10 - 4, 0), window->DC.CursorPos + ImVec2(w + 4, label_size.x > 0 ? label_size.y + 10 : 10) + ImVec2(16 - 6, 0 + 1));
    ...
    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min, ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), label);
    ...
    if (IsItemHovered())
        window->DrawList->AddRectFilledMultiColor(slider_bb.Min, slider_bb.Max, ImColor(79 / 255.f, 79 / 255.f, 79 / 255.f, g.Style.Alpha), ImColor(79 / 255.f, 79 / 255.f, 79 / 255.f, g.Style.Alpha), ImColor(46 / 255.f, 46 / 255.f, 46 / 255.f, g.Style.Alpha), ImColor(46 / 255.f, 46 / 255.f, 46 / 255.f, g.Style.Alpha));
    else
        window->DrawList->AddRectFilledMultiColor(slider_bb.Min, slider_bb.Max, ImColor(69 / 255.f, 69 / 255.f, 69 / 255.f, g.Style.Alpha), ImColor(69 / 255.f, 69 / 255.f, 69 / 255.f, g.Style.Alpha), ImColor(36 / 255.f, 36 / 255.f, 36 / 255.f, g.Style.Alpha), ImColor(36 / 255.f, 36 / 255.f, 36 / 255.f, g.Style.Alpha));
    ...
    window->DrawList->AddRectFilledMultiColor(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha), ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha));
    window->DrawList->AddRect(slider_bb.Min, slider_bb.Max, ImColor(3 / 255.f, 3 / 255.f, 3 / 255.f, g.Style.Alpha));
    ...
    PushFont(GetIO().Fonts->Fonts[1]);
    window->DrawList->AddText(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - textSize.x / 2, slider_bb.Max.y - textSize.y / 2 - 1), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), value_buf);
	PopFont();
...
bool ui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = CalcItemSize(size_arg, window->Size.x - 48 + 6, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(16 - 6, 0), window->DC.CursorPos + size);
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	if (!is_multiline)
	{
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), ImColor(41 / 255.f, 41 / 255.f, 41 / 255.f, g.Style.Alpha));
		//window->DrawList->AddRect(frame_bb.Min + ImVec2(2, 2), frame_bb.Max - ImVec2(2, 2), ImColor(1, 1, 1, 255));
	...
	if (g.ActiveId == id || is_currently_scrolling)
	{
		...
		if (is_multiline || buf_display_len < buf_display_max_length)
			draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - render_scroll, ImColor(190 / 255.f, 190 / 255.f, 190 / 255.f, g.Style.Alpha), buf_display, buf_display + buf_display_len, 0.0f, is_multiline ? NULL : &clip_rect);
	...
	else
	{
		...
		if (is_multiline || (buf_end - buf_display) < buf_display_max_length)
			draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, ImColor(190 / 255.f, 190 / 255.f, 190 / 255.f, g.Style.Alpha), buf_display, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
	}
...
void ui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ui::ColorButton2(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    auto gradient = GetColorU32(col_source) - ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, ImClamp(120.f / 255.f, 0.f, g.Style.Alpha));
    ...
    if (g.Style.FrameBorderSize > 0.0f)
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha), rounding); // Color button are often in need of some sort of border
...
bool ui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
   ...
   // if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    //window->DrawList->AddRect(ImVec2(200, 200), ImVec2(200, 200), ImColor(1 / 255.f, 1 / 255.f, 1 / 255.f, g.Style.Alpha));

   // draw_list->AddRectFilled(picker_pos - ImVec2(300, 16), picker_pos + ImVec2(1500, 500), ImColor(0.f, 0.f, 0.f, 1.f));
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar) {
        // Alpha bar logic
        if (alpha_bar) {
            SetCursorScreenPos(ImVec2(bar1_pos_x + 1, picker_pos.y + 1));
    ...
    if (alpha_bar) {
        ...
        //draw_list->AddRectFilled(bar1_bb.Min, bar1_bb.Max, col32_no_alpha, col32_no_alpha, col32_no_alpha & ~IM_COL32_A_MASK);
...
//    //    if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
...
//    //            value_changed = true;
//    //        }
//    //    if (flags & ImGuiColorEditFlags_HSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
//    //        value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_HSV);
//    //    if (flags & ImGuiColorEditFlags_HEX || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
...
bool ui::ColorButton(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        window->DrawList->AddRect(bb_inner.Min - ImVec2(1, 1), bb_inner.Max + ImVec2(1, 1), ImColor(0.f, 0.f, 0.f, g.Style.Alpha));
        ...
        window->DrawList->AddRectFilledMultiColor(bb_inner.Min, bb_inner.Max,
            ...
            ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha / 2), ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha / 2));
    }
    ...
    // NB: The ActiveId test is merely an optional micro-optimization, BeginDragDropSource() does the same test.
        ...
        if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
        {
            ...
            SetCursorPosX(10 - 4 + 1 - 4);
            ColorButton(desc_id, col, flags);
            SameLine();

            /*std::stringstream ss;
            ss << std::to_string((int)(col.x * 255.f)) << ", " << std::to_string((int)(col.y * 255.f)) << ", " << std::to_string((int)(col.z * 255.f)) << ", " << std::to_string((int)(col.w * 255.f));
            SetCursorPosX(10 - 4 + 1);
...
void ui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton("RGB", (opts & ImGuiColorEditFlags_RGB) != 0)) opts = (opts & ~ImGuiColorEditFlags__DisplayMask) | ImGuiColorEditFlags_RGB;
        if (RadioButton("HSV", (opts & ImGuiColorEditFlags_HSV) != 0)) opts = (opts & ~ImGuiColorEditFlags__DisplayMask) | ImGuiColorEditFlags_HSV;
        if (RadioButton("HEX", (opts & ImGuiColorEditFlags_HEX) != 0)) opts = (opts & ~ImGuiColorEditFlags__DisplayMask) | ImGuiColorEditFlags_HEX;
    }
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered)
    {
        // todo: simv0l - fix size of this shit (min.x -= 5, max.x += 5).
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha));
        PushFont(GetIO().Fonts->Fonts[1]);
    }

    window->DrawList->PushClipRect(bb.Min, bb.Max, true);
    window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? ImColor(settings::m_menu_color[0], settings::m_menu_color[1], settings::m_menu_color[2], g.Style.Alpha) : ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), label);
    window->DrawList->PopClipRect();
...
bool ui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 size = CalcItemSize(size_arg , window->Size.x - 64 - 12, GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
    ImVec2 frame_size = ImVec2(size.x, ImMax(size.y, label_size.y));
    ImRect frame_bb(window->DC.CursorPos + ImVec2(16 + 6, 0), window->DC.CursorPos + frame_size);
    ImRect bb(frame_bb.Min + ImVec2(16 + 6, 0 ), frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
