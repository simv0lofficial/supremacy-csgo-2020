...
struct ImVec3
{
    ...
    float  operator[] (size_t idx) const { IM_ASSERT(idx <= 2); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
    float& operator[] (size_t idx) { IM_ASSERT(idx <= 2); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
...
namespace ui
{
    ...
    IMGUI_API bool          ButtonText(const char* label, const ImVec2& size = ImVec2(0,0));    // button
    ...
    IMGUI_API bool          Checkbox(const char* label, bool* v, bool safe = true);
    ...
    IMGUI_API bool          BeginCombo2(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int items = 1);
    ...
    IMGUI_API bool          SingleSelect2(const char* label, int* current_item, std::vector<const char*> items);
    ...
    IMGUI_API bool          ColorButton2(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0));  // display a colored square/button, hover for details, return true when pressed.
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    IMGUI_API void          TextEx(const char* text, const char* text_end = NULL, ImGuiTextFlags flags = 0);                 // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API void          TabButton2(const char* label2, int* currenttab, int number, int total);
    IMGUI_API void          TabButton3(const char* label, const char* label2, int* selected, int num, int total);

    // Logging/Capture
    // - All text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging::get().
...
enum ImGuiWindowFlags_
{
    ...
    // [Custom]
    ImGuiWindowFlags_Confirmation = 1 << 21,
	...
	ImGuiWindowFlags_MainWindow				= 1 << 30,	
...
struct ImGuiStyle
{
    ...
    float       Alpha2;                      // Global alpha applies to everything in ImGui.
