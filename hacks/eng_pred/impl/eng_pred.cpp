...
namespace supremacy::hacks {
	...
	void c_eng_pred::predict( valve::user_cmd_t* const user_cmd ) {
		...
		*g_context->addresses( ).m_pred_seed = user_cmd->m_random_seed;
		...
		// forcing CCSGameMovement::CheckJumpButton to call C_CSPlayer::DoAnimationEvent to trigger the jumping animation.
		valve::g_local_player->use_new_anim_state( ) = true;
		...
		// UpdateCollisionBounds.
		{
			if ( valve::g_local_player->flags( ) & valve::e_ent_flags::ducking )
				valve::g_local_player->set_collision_bounds( { -16.f, -16.f, 0.f }, { 16.f,  16.f, 54.f } );
			else
				valve::g_local_player->set_collision_bounds( { -16.f, -16.f, 0.f }, { 16.f,  16.f, 72.f } );
		}
