...
namespace supremacy::hacks {
	...
	void c_aim_bot::scan_point_mt(scan_point_data* data) {
		const aim_target_t& target = data->target;
		aim_point_t& point = data->point;
		const bool ignore_dmg = data->ignore_dmg;
		...
		if (!ignore_dmg) {
			...
			if (is_taser)
				min_dmg = target.m_entry->m_player->health() + 5;
		...
		const auto& negative_low_bones = target.m_lag_record->m_sides_low.at(0).m_bones;
		const auto& positive_low_bones = target.m_lag_record->m_sides_low.at(1).m_bones;
		...
		if (zero_bones[hitbox->m_bone].m_matrix[0][3] != negative_bones[hitbox->m_bone].m_matrix[0][3]
			...
			&& zero_bones[hitbox->m_bone].m_matrix[0][3] != negative_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& zero_bones[hitbox->m_bone].m_matrix[0][3] != positive_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& negative_bones[hitbox->m_bone].m_matrix[0][3] != negative_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& negative_bones[hitbox->m_bone].m_matrix[0][3] != positive_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& negative_bones[hitbox->m_bone].m_matrix[0][3] != positive_bones[hitbox->m_bone].m_matrix[0][3]
			&& positive_bones[hitbox->m_bone].m_matrix[0][3] != negative_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& positive_bones[hitbox->m_bone].m_matrix[0][3] != positive_low_bones[hitbox->m_bone].m_matrix[0][3]
			&& negative_low_bones[hitbox->m_bone].m_matrix[0][3] != positive_low_bones[hitbox->m_bone].m_matrix[0][3]) {
			if (intersect(shoot_pos, point.m_pos, hitbox, zero_bones[hitbox->m_bone])) {
				++point.m_intersections;
				++point.m_intersections_low;
			}
			...
			if (intersect(shoot_pos, point.m_pos, hitbox, negative_low_bones[hitbox->m_bone]))
				++point.m_intersections_low;

			if (intersect(shoot_pos, point.m_pos, hitbox, positive_low_bones[hitbox->m_bone]))
				++point.m_intersections_low;
		}
	...
	void c_aim_bot::scan_center_points(aim_target_t& target, const int hitgroups) const {
		const auto& anim_side = target.m_lag_record->m_side < 3 ? target.m_lag_record->m_sides.at(target.m_lag_record->m_side) : target.m_lag_record->m_sides_low.at(target.m_lag_record->m_side - 3);
	...
	void c_aim_bot::scan_points(
		...
		for (std::size_t i{}; i < m_hitgroups.size(); ++i) {
			...
			if (((1 << i) & multi_points)) {
				const auto scale = (((i ? sdk::g_config_system->body_scale : sdk::g_config_system->head_scale) / 100.f * 0.7f) + 0.2f);
		...
		std::vector < scan_point_data > m_data;

		for (auto& point : target.m_points) {
			scan_point_data* data = &m_data.emplace_back();
			data->target = target;
			data->point = point;
			data->ignore_dmg = false;
			Threading::QueueJobRef(scan_point_mt, data);
		}
	...
	bool c_aim_bot::select_points(aim_target_t& target) const {
		...
		for (auto& point : target.m_points) {
			...
			if ((target.m_lag_record->m_flags & valve::e_ent_flags::on_ground)
				...
				if (point.m_intersections_low == 2) {
					if (sdk::g_config_system->safe_point_conditions[0]
						&& pen_data.m_dmg > min_dmg) {
						best_point = &point;
						continue;
	...
	std::optional< aim_record_t > c_aim_bot::extrapolate(const player_entry_t& entry) const
	{
		...
		const auto delay = g_movement->should_fake_duck()
			? 15 - valve::g_client_state->m_choked_cmds : 0;
	...
	std::optional< aim_record_t > c_aim_bot::select_record(const player_entry_t& entry) const {
		...
		for (auto i = entry.m_lag_records.rbegin(); i != entry.m_lag_records.rend(); i = std::next(i)) {
			...
			if (!lag_record->m_dormant) {
				...
				scan_center_points(target, cfg_hitgroups);
				...
				if ((lag_record->m_flags & valve::e_ent_flags::on_ground)
					...
					if (target.m_best_point->m_intersections_low != best_point.value().m_intersections_low) {
						if (target.m_best_point->m_intersections_low > best_point.value().m_intersections_low) {
							best_record = lag_record;
							best_point = *target.m_best_point;
						}
	...
	aim_point_t* c_aim_bot::select_point(const aim_target_t& target, const int cmd_number) const {
		...
		if (sdk::g_config_system->body_aim_conditions[1]
			&& target.m_best_point->m_intersections < 3)
			return target.m_best_body_point;
	...
	void c_aim_bot::scan_targets() {
		...
		for (auto& target : m_targets)
			scan_points(target, m_hitgroups, m_multi_points, false);
	...
	void c_aim_bot::on_create_move(valve::user_cmd_t& user_cmd) {
		...
		m_is_peeking = onpeek_called >= valve::g_global_vars->m_cur_time
			&& onpeek_called < valve::g_global_vars->m_cur_time + valve::to_time((((*valve::g_game_rules)->is_valve_ds() == true) ? 7 : 15));

		if (!g_context->weapon() || !g_context->wpn_data())
			return;

		if (peek_correction()
			&& onpeek_called < valve::g_global_vars->m_cur_time)
			onpeek_called = valve::g_global_vars->m_cur_time + valve::to_time((((*valve::g_game_rules)->is_valve_ds() == true) ? 6 : 14));
		...
		if (g_context->flags() & e_context_flags::can_shoot) {
			...
			else if (const auto hit_chance = calc_hit_chance(target, angle, point); hit_chance >= (item_index == valve::e_item_index::taser ? 70 : sdk::g_config_system->minimum_hitchance)) {
	...
	bool c_aim_bot::peek_correction() const {
		if (!(valve::g_local_player->flags() & valve::e_ent_flags::on_ground) || valve::g_local_player->velocity().length() < 5.f)
			return false;

		vec3_t backup_shoot_pos = g_context->shoot_pos();
		g_context->shoot_pos() += (valve::g_local_player->velocity() * valve::g_global_vars->m_interval_per_tick) * 7.f;
		...
		for (auto i = 0; i < valve::g_global_vars->m_max_clients; ++i) {
			...
			target.m_entry = const_cast<player_entry_t*>(&entry);
			target.m_lag_record = entry.m_lag_records.back();
			const auto hp = target.m_entry->m_player->health();
			scan_center_points(target, cfg_hitgroups);
			...
			for (auto point : target.m_points) {
				if (point.m_pen_data.m_dmg) {
					auto min_dmg = point.m_pen_data.m_remaining_pen == 4 ? sdk::g_config_system->minimum_damage : sdk::g_config_system->penetration_minimum_damage;
